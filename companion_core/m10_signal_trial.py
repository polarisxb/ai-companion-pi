"""M10.2 supervised real Signal send trial gate.

Runs one bounded receive/reply pass through the Signal chat bridge after
verifying M10.1 dry-run evidence, upstream M7/M8/M9 freezes, chat config, and
the explicit operator confirmation. The trial passes only when at least one
allowlisted inbound message received a reply and nothing failed.

Tests inject a fake transport and dialogue runner; the CLI wires signal-cli
and the real provider on the Raspberry Pi.
"""
...
READY_RECOMMENDATION = "m10_signal_trial_ready"
M10_DRY_RUN_RECOMMENDATION = "m10_signal_dry_run_ready"


@dataclass
class M10SignalTrialResult:
    ok: bool
    recommendation: str
    report: dict
...
def run_m10_signal_trial(
    ...
    *,
    ...
    """Run the bounded supervised Signal reply trial."""
    ...
    stages.append(_dry_run_stage(dry_run_report))
    ...
    try:
        ...
        stages.append(_stage(
            ...
            details={
                ...
                "daily_reply_budget": config.daily_reply_budget,
                "max_replies_per_poll": config.max_replies_per_poll,
            },
        ))
    ...
    stages.append(_stage(
        ...
        details={"transport": transport_name},
    ))
    ...
    if _all_pass(stages):
        ...
        try:
            # run_loop holds the single-instance bridge lock, so a trial can
            # never race an already-activated M10.3 listener service.
            attempts.extend(bridge.run_loop(max_polls=max_polls))
            ...
            stages.append(_stage(
                "trial_execution",
                False,
                f"trial execution raised {type(exc).__name__}: {exc}",
            ))
    else:
        stages.append(_stage(
            "trial_execution",
            False,
            "trial execution skipped because preflight failed",
        ))

    stages.append(_trial_boundary_stage(attempts))
    ...
    report = {
        ...
        "profile": {
            "name": "M10 supervised signal send trial",
    ...
    return M10SignalTrialResult(ok=ok, recommendation=report["recommendation"], report=report, errors=errors)


def write_m10_signal_trial_report(
    paths: CompanionPaths,
    report: dict,
    ...
    report_path = (
        ...
        if report_file
        else paths.life_loop_dir / "m10_signal_trial_report.json"
    )
...
def _dry_run_stage(report: dict | None) -> dict:
    problems = []
    if not isinstance(report, dict):
        problems.append("M10.1 dry-run report is missing or invalid")
    else:
        if report.get("ok") is not True:
            problems.append("M10.1 ok is not true")
        if report.get("milestone") != "M10.1":
            problems.append("milestone is not M10.1")
        if report.get("recommendation") != M10_DRY_RUN_RECOMMENDATION:
            problems.append(f"recommendation is not {M10_DRY_RUN_RECOMMENDATION}")
        if report.get("stop_reasons"):
            problems.append("M10.1 report has stop_reasons")
    return _stage(
        "m10_dry_run_ready",
        not problems,
        "M10.1 dry-run evidence is ready" if not problems else "; ".join(problems),
    )
...
def _trial_execution_stage(attempts: list[dict]) -> dict:
    ...
    if problems:
        return _stage("trial_execution", False, "; ".join(problems), details={"decision_counts": counts})
    return _stage(
        "trial_execution",
        True,
        f"trial replied to {counts['replied']} message(s) with no failures",
        details={"decision_counts": counts},
    )


def _trial_boundary_stage(attempts: list[dict]) -> dict:
    problems = []
    for attempt in attempts:
        ...
        boundaries = attempt.get("boundaries") or {}
        if boundaries.get("proactive_outbound_sent") is not False:
            problems.append("attempt boundary proactive_outbound_sent must be false")
    if problems:
        return _stage("trial_boundary", False, "; ".join(sorted(set(problems))))
    return _stage(
        "trial_boundary",
        True,
        "trial replies stayed reactive, labeled, and evidence-backed",
