#!/usr/bin/env python3
"""Run the M6.3 controlled real-Pi manual wake trial."""
...
from companion_core import CompanionPaths, run_m6_pi_manual_wake_trial


def main() -> int:
    parser = argparse.ArgumentParser(description="Run M6.3 controlled real-Pi manual wake trial")
    parser.add_argument("--companion-home", default=None, help="Target CompanionHome")
    parser.add_argument(
        "--confirm-real-pi-wake",
        action="store_true",
        help="Explicitly permit the real Pi manual wake after all M6.3 guards pass.",
    )
    parser.add_argument(
        "--preflight-report",
        default=None,
    ...
    parser.add_argument("--trigger", default="m6-pi-manual-wake", help="Trigger prefix for the manual wake trial")
    ...
    parser.add_argument(
        ...
        choices=(1, 2),
        default=2,
        help="Maximum attempts. M6.3 permits at most one infrastructure retry.",
    )
    parser.add_argument(
        "--no-write-report",
        action="store_true",
        help="Do not write life-loop/m6_pi_manual_wake_report.json.",
    )
    parser.add_argument(
        "--report-file",
        default=None,
        help="Override report path. Defaults to life-loop/m6_pi_manual_wake_report.json.",
    )
    ...
    report = run_m6_pi_manual_wake_trial(
        paths,
        confirm_real_pi_wake=args.confirm_real_pi_wake,
