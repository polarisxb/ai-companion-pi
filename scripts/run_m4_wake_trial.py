#!/usr/bin/env python3
"""Run the M4.3 manual DeepSeek wake trial wrapper."""
...
from companion_core import CompanionPaths, run_m4_wake_trial


def main() -> int:
    parser = argparse.ArgumentParser(description="Run M4.3 manual wake trial")
    parser.add_argument("--companion-home", default=None, help="Target CompanionHome")
    parser.add_argument("--trigger", default="m4-pi-manual-wake", help="Trigger prefix for the manual wake trial")
    ...
    parser.add_argument(
        ...
        choices=(1, 2),
        default=2,
        help="Maximum attempts. M4 policy permits at most one infrastructure retry.",
    )
    parser.add_argument(
        "--no-write-report",
        action="store_true",
        help="Do not write life-loop/m4_wake_trial_report.json.",
    )
    parser.add_argument(
        "--report-file",
        default=None,
        help="Override report path. Defaults to life-loop/m4_wake_trial_report.json.",
    )
